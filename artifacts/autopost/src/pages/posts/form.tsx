        ...
        <div className="space-y-6">
                ...
                <ToggleGroup type="multiple" value={platforms} onValueChange={(v) => { if(v.length) setPlatforms(v) }} className="flex flex-wrap gap-2 justify-start">
                  {PLATFORMS.map(p => (
                    <ToggleGroupItem 
                      key={p} 
                      value={p} 
                      className={`w-12 h-12 rounded-xl border-2 transition-all ${platforms.includes(p) ? 'border-primary bg-primary/10' : 'border-transparent bg-muted/50 hover:bg-muted'}`}
                      aria-label={`Toggle ${p}`}
                    >
                      <PlatformIcon platform={p} className={`w-5 h-5 ${platforms.includes(p) ? 'text-primary' : 'text-muted-foreground'}`} />
                    </ToggleGroupItem>
                  ))}
                </ToggleGroup>
              </div>
          ...
          <div className="flex gap-3 pt-4">
            ...
            <Button 
              ...
              disabled={createPost.isPending || updatePost.isPending}
            >
              {createPost.isPending ? 'Saving...' : 'Schedule'}
            </Button>
          </div>
        </div>
      </div>
