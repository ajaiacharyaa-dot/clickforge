...
export const ThumbnailPreview: React.FC<ThumbnailPreviewProps> = ({ variations, onStyleChange }) => {
  ...
  return (
    <div className="w-full">
      <label className="block text-sm font-semibold text-gray-700 mb-3">Generated Thumbnails</label>

      {variations.length > 0 ? (
        <div className="space-y-6">
          <div className="bg-gray-100 rounded-lg overflow-hidden aspect-video relative">
            {/* Use native <img> for robust handling of fetch URLs */}
            <img src={variations[selectedVariant].image_url} alt={`Variant ${selectedVariant + 1}`} className="object-cover w-full h-full" />
          </div>

          <div className="grid grid-cols-3 gap-3">
            {variations.map((variation, index) => (
              <button key={index} onClick={() => setSelectedVariant(index)} className={`p-2 rounded-lg border-2 transition ${selectedVariant === index ? 'border-primary' : 'border-gray-200'}`}>
                <div className="w-full aspect-video mb-2 bg-gray-100 rounded overflow-hidden">
                  <img src={variation.image_url} alt={`Variant ${index + 1}`} className="object-cover w-full h-full" />
                </div>
